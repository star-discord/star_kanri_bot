...
module.exports = {
  ...
  handle: async function(interaction) {
    ...
    if (!result.success) {
      ...
      if (!interaction.replied && !interaction.deferred) {
        return interaction.reply({ content: '退勤処理に失敗しました。', flags: MessageFlagsBitField.Ephemeral });
      }
      return;
    }
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: '退勤を記録しました。', flags: MessageFlagsBitField.Ephemeral });
    }
  }
};
