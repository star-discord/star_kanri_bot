...
for (const file of files) {
  ...
  try {
    ...
    if (typeof handler.handle !== 'function') {
      console.warn(`⚠️ ChatGPTボタンモジュールに handle 関数がありません: ${file}`);
      continue;
    }
    ...
    if (typeof handler.customId === 'string') {
      ...
      console.warn(`⚠️ ChatGPTボタンモジュールに customId/customIdStart が未定義: ${file}`);
    }
  } catch (err) {
...
/**
 * customId に対応するChatGPTボタンハンドラを探す（完全一致 → 前方一致）
...
function findHandler(customId) {
  ...
  console.warn(`⚠️ 対応するChatGPTボタンハンドラが見つかりません: ${customId}`);
  return null;
}
