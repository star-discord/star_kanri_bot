...
module.exports = {
  async handle(interaction, uuid) {
    ...
    if (!instance) {
      return await interaction.reply({ content: '⚠ 該当の凸スナ情報が見つかりません。', flags: InteractionResponseFlags.Ephemeral });
    }

    const newBody = interaction.fields.getTextInputValue('body')?.trim();
    if (!newBody || newBody.length === 0) {
      return await interaction.reply({ content: '❌ 本文が空です。', flags: InteractionResponseFlags.Ephemeral });
    }
