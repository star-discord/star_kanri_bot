...
module.exports = {
  customIdStart: 'totsusuna_report_button_', // ✁E命名規則に従って統一

  /**
   * 凸スナ報告�Eタン押下時の処琁EↁEモーダルを表示
   * @param {import('discord.js').ButtonInteraction} interaction
   */
  async handle(interaction) {
    const customId = interaction.customId;

    // ボタンIDからUUIDを抽出�E�侁E totsusuna_report_button_<UUID>�E�E    const uuid = customId.split('_').slice(-1)[0];

    const modal = new ModalBuilder()
      .setCustomId(`totsusuna_modal_${uuid}`) // uuidを付与して特宁E      .setTitle('📝 凸スナ報呁E);
