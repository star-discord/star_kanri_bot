...
module.exports = {
  async handle(interaction, uuid) {
    ...
    if (!instance) {
      return await interaction.reply({ content: '⚠ 対象の凸スナ設置が見つかりません。', ephemeral: true });
    }
    ...
    if (!channel) {
      return await interaction.reply({ content: '⚠ チャンネルが見つかりません。', ephemeral: true });
    }
    ...
    await interaction.reply({ content: '✅ 凸スナを再送信しました。', ephemeral: true });
  },
};
