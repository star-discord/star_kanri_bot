...
for (const file of files) {
  ...
  try {
    delete require.cache[require.resolve(modulePath)]; // 髢狗匱荳ｭ縺ｮ繧ｭ繝｣繝・す繝･辟｡蜉ｹ蛹・    const handler = require(modulePath);

    if (typeof handler.handle !== 'function') {
      console.warn(`笞・・繝｢繝ｼ繝繝ｫ繝｢繧ｸ繝･繝ｼ繝ｫ縺ｫ handle 髢｢謨ｰ縺後≠繧翫∪縺帙ｓ: ${file}`);
      continue;
    }
    ...
    if (typeof handler.customId === 'string') {
      ...
      console.warn(`笞・・繝｢繝ｼ繝繝ｫ繝｢繧ｸ繝･繝ｼ繝ｫ縺ｫ customId/customIdStart 縺梧悴螳夂ｾｩ: ${file}`);
    }

  } catch (err) {
    console.warn(`笶・繝｢繝ｼ繝繝ｫ繝輔ぃ繧､繝ｫ縺ｮ隱ｭ縺ｿ霎ｼ縺ｿ縺ｫ螟ｱ謨・(${file}):`, err);
  }
}

/**
 * 繝｢繝ｼ繝繝ｫ customId 縺ｫ蟇ｾ蠢懊☆繧九ワ繝ｳ繝峨Λ繧呈爾縺呻ｼ亥ｮ悟・荳閾ｴ 竊・蜑肴婿荳閾ｴ・・ * @param {string} customId
 * @returns {object|null}
 */
function findHandler(customId) {
  ...
  console.warn(`笞・・蟇ｾ蠢懊☆繧九Δ繝ｼ繝繝ｫ繝上Φ繝峨Λ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ: ${customId}`);
  return null;
}
